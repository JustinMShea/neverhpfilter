...
Because it an xts object, it can be easily graphed with `plot.xts`, 
the task beautifully with a simple function call. Note the `panels` arguement,
which we used to plot bars of the cycle.
...
Once can use packages which recognize xts objects such as `PerformanceAnalytics`.
Recession bars are created easily with the `period.areas` arguement of the
...
employment_cycle <- yth_cycle(Employment_log, h = 24, p = 12)
