...
```{r, message=FALSE, warning=FALSE}
fig6_data <- 100*log(merge(GDPC1, EXPGSC1, PCECC96, IMPGSC1, GDPIC1, GCEC1)["1947/2016-1"])

fig6_plots <- function(x, ...) {
               cycle <-  yth_filter(x, h = 8, p = 4, output = c("cycle", "random"), family = gaussian)
              main <- paste0(names(cycle)[1]," and random")
              plot(cycle, grid.col = "white", lwd=1, main = main)
}
```

```{r, warning=FALSE, message=FALSE, eval=FALSE}
par(mfrow=c(3,2))
lapply(fig6_data, fig6_plots)
```
...
When striving to recreate a statistical method found in a journal or paper, one can perform surprisingly well by thoroughly digesting the relevant sections and "eyeballing" the output graphs usually included in the authors work. Better still, is a table presenting the authors results, which one may use to compare against their own reproduction.

Fortunately for us, the Appendix displays such a table. I compare Hamilton's results with those estimated by functions contained in the **`neverhpfilter`** package.

Below are Hamilton's results from table 2 (Hamilton 40), which are stored as a `data.frame` in this package.
...
```{r}
kable(Hamilton_table_2, align = 'l', caption = "Hamilton's results: table 2, pg. 40")
```
...
```{r, warning = FALSE, message = FALSE}
# Merge series with default quarterly frequency
quarterly_data <- 100*log(merge(GDPC1, PCECC96, GDPIC1, EXPGSC1, IMPGSC1, GCEC1, GDPDEF)["1947/2016-1"])

# Transform monthly to quarterly series and match Hamiltons date range for the series.
monthly_data <- merge(100*log(PAYEMS), 100*log(SP500$SP500), UNRATENSA, GS10, FEDFUNDS)
to_quarterly_data <- do.call(merge, lapply(monthly_data, to.quarterly, OHLC = FALSE))["1947/2016-6"]

# Merge series, and log transform monthly to quarterly series.
quarterly_data <- merge(quarterly_data, to_quarterly_data)

# Calculate cycle and correlation
...
# Calculate random and correlation
...
my_table_2 <- round(data.frame(cbind(cycle.sd, GDP.cor, random.sd, random.cor)), 4)
names(my_table_2) <- names(Hamilton_table_2[,-5])

kable(my_table_2, digits = 2)
```


```{r, warning = FALSE, message = FALSE, eval = FALSE}
 # Real Gross Domestic Product
        GDP_f <- gdp_cycle["/2016-3"]
...
colnames(GDP) <- names(Hamilton_table_2)
 ...
 # Employment Situation
        employ_f <- employ_cycle["/2016-6"]

   employ.St.Dev <- round(sd(employ_f[,1], na.rm=TRUE), 4)
 employ.GDPCorr. <- round(cor(employ_f[,1], gdp_cycle[,1]["/2016-6"], use = "complete.obs"), 4)
   employ.Random <- round(sd(employ_f[,2], na.rm=TRUE), 3)
 employ.RanCorr. <- round(cor(employ_f[,2], gdp_cycle[,2]["/2016-6"], use = "complete.obs"), 4)
...
```{r, eval=FALSE, warning = FALSE, message = FALSE, caption = "Comparison table: Hamilton vs neverhpfilter"}
 # Combined table
kable(rbind(Hamilton_table_2, my_table_2), align = 'l', caption = "Hamilton's table 2 compared with estimates from neverhpfilter::yth_filter")
